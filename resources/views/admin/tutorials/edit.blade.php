<x-admin-layout>
    <h2 class="text-xl font-semibold mb-4">Edit Tutorial: {{ $tutorial->title }}</h2>

    <form action="{{ route('admin.tutorials.update', $tutorial->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-sm">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="title" class="block text-gray-700 text-sm font-bold mb-2">Tutorial Title:</label>
            <input type="text" name="title" id="title" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('title') border-red-500 @enderror" value="{{ old('title', $tutorial->title) }}" required>
            @error('title')
        ...
        <div class="mb-4">
            ...
            <textarea name="summary" id="summary" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('summary') border-red-500 @enderror">{{ old('summary', $tutorial->summary) }}</textarea>
            @error('summary')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="pre_quiz_id" id="pre_quiz_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('pre_quiz_id') border-red-500 @enderror">
                ...
                @foreach($quizzes as $quiz)
                    <option value="{{ $quiz->id }}" {{ old('pre_quiz_id', $tutorial->pre_quiz_id) == $quiz->id ? 'selected' : '' }}>{{ $quiz->title }}</option>
                @endforeach
            </select>
            @error('pre_quiz_id')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
        ...
        <div class="mb-4">
            ...
            <select name="post_quiz_id" id="post_quiz_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('post_quiz_id') border-red-500 @enderror">
                ...
                @foreach($quizzes as $quiz)
                    <option value="{{ $quiz->id }}" {{ old('post_quiz_id', $tutorial->post_quiz_id) == $quiz->id ? 'selected' : '' }}>{{ $quiz->title }}</option>
                @endforeach
            </select>
            @error('post_quiz_id')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
        ...
        <div class="mb-4">
            ...
            <select name="content_type" id="content_type" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('content_type') border-red-500 @enderror" required>
                <option value="text" {{ old('content_type', $tutorial->content_type) == 'text' ? 'selected' : '' }}>Text</option>
                <option value="image" {{ old('content_type', $tutorial->content_type) == 'image' ? 'selected' : '' }}>Image</option>
                <option value="video" {{ old('content_type', $tutorial->content_type) == 'video' ? 'selected' : '' }}>Video (URL)</option>
                <option value="mixed" {{ old('content_type', $tutorial->content_type) == 'mixed' ? 'selected' : '' }}>Mixed (Text, Image, Video)</option>
            </select>
            @error('content_type')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        ...
        <div id="text-content-fields" class="mb-4" style="display: {{ old('content_type', $tutorial->content_type) == 'text' || old('content_type', $tutorial->content_type) == 'mixed' ? 'block' : 'none' }};">
            <label for="content_text" class="block text-gray-700 text-sm font-bold mb-2">Tutorial Text Content:</label>
            <textarea name="content_text" id="content_text" rows="8" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('content_text') border-red-500 @enderror">{{ old('content_text', $tutorial->content_text) }}</textarea>
            @error('content_text')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>

        <div id="image-content-fields" class="mb-4" style="display: {{ old('content_type', $tutorial->content_type) == 'image' || old('content_type', $tutorial->content_type) == 'mixed' ? 'block' : 'none' }};">
            <label for="image" class="block text-gray-700 text-sm font-bold mb-2">Upload New Image (Leave blank to keep current):</label>
            ...
            @if($tutorial->image_path)
                ...
                <img src="{{ asset('storage/' . $tutorial->image_path) }}" alt="Current image" class="mt-2 max-w-xs h-auto rounded">
            @endif
        </div>

        <div id="video-content-fields" class="mb-4" style="display: {{ old('content_type', $tutorial->content_type) == 'video' || old('content_type', $tutorial->content_type) == 'mixed' ? 'block' : 'none' }};">
            <label for="video_url" class="block text-gray-700 text-sm font-bold mb-2">Video URL (e.g., YouTube link):</label>
            <input type="url" name="video_url" id="video_url" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('video_url') border-red-500 @enderror" value="{{ old('video_url', $tutorial->video_url) }}">
            @error('video_url')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="order" id="order" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('order') border-red-500 @enderror" value="{{ old('order', $tutorial->order) }}">
            @error('order')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
