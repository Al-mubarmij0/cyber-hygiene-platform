<x-admin-layout>
    <h2 class="text-xl font-semibold mb-4">Create New Question</h2>

    <form action="{{ route('admin.questions.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-sm">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <textarea name="question_text" id="question_text" rows="4" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('question_text') border-red-500 @enderror" required>{{ old('question_text') }}</textarea>
            @error('question_text')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
