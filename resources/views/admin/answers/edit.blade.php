<x-admin-layout>
    <h2 class="text-xl font-semibold mb-4">Edit Answer: {{ Str::limit($answer->answer_text, 50) }}</h2>

    <form action="{{ route('admin.answers.update', $answer->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="question_id" id="question_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('question_id') border-red-500 @enderror" required>
                ...
                @foreach($questions as $question)
                    <option value="{{ $question->id }}" {{ old('question_id', $answer->question_id) == $question->id ? 'selected' : '' }}>{{ Str::limit($question->question_text, 80) }}</option>
                @endforeach
            </select>
            @error('question_id')
        ...
        <div class="mb-4">
            ...
            <textarea name="answer_text" id="answer_text" rows="2" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('answer_text') border-red-500 @enderror" required>{{ old('answer_text', $answer->answer_text) }}</textarea>
            @error('answer_text')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
