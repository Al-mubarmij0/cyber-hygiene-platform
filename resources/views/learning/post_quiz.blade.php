<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Post-Quiz for: ') }} {{ $tutorial->title }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h4 class="text-lg font-bold mb-4">Please complete this short quiz after the tutorial.</h4>
                    @if($quiz->questions->isEmpty())
                        <p class="text-gray-600">This quiz has no questions yet. Please contact an administrator.</p>
                    @else
                        <form action="{{ route('learn.submit_post_quiz', $tutorial) }}" method="POST">
                            @csrf
